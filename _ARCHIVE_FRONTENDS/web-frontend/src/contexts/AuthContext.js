...
export const AuthProvider = ({ children }) => {
  ...
  const [user, setUser] = useState(isDevelopment ? {
    id: 1,
    email: 'dev@drumtrackai.com',
    name: 'Development User',
    tier: 'expert'
  } : null);
  const [token, setToken] = useState(isDevelopment ? 'dev_token' : null);
  const [isAuthenticated, setIsAuthenticated] = useState(isDevelopment); // Auto-authenticated for dev
  const [isLoading, setIsLoading] = useState(false);
  const [tier, setTier] = useState(isDevelopment ? 'expert' : 'basic'); // Set to expert tier for full feature access
  const [usage, setUsage] = useState(isDevelopment ? { current: 5, limit: -1 } : { current: 0, limit: 10 }); // Unlimited usage for expert
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        throw new Error('Authentication not configured for production. Please set up a backend auth service.');
      }
    } catch (error) {
  ...
  const register = async (email, password, name) => {
    ...
    try {
      // Demo registration for development only
      if (process.env.NODE_ENV !== 'production') {
        ...
        throw new Error('Registration not configured for production. Please set up a backend auth service.');
      }
    } catch (error) {
