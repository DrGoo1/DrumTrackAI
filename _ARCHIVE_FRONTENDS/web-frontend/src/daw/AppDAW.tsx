...
export const AppDAW: React.FC = () => {
  ...
  const { song, addTrack, addClip, updateNotes, getClip, updateClip, setTempoMap } = useMidi()
  ...
  function convertDrumTrackNotesToMidi(drumTrack: any): MidiNote[] {
    if (!drumTrack?.notes?.length) return []

    const resolution = Number(drumTrack.resolution_ppq) || 960
    const beatsPerBar = song.timeSig?.[0] ?? 4
    const ppq = Number(song.ppq) || 480
    const ppqRatio = ppq / resolution
    const ticksPerBarSource = beatsPerBar * resolution

    return drumTrack.notes.map((note: any, idx: number): MidiNote => {
      const barIndex = Number(note.barIndex ?? 0)
      const tickInBar = Number(note.tickInBar ?? 0)
      const tickLength = Number(note.tickLength ?? resolution / 4)
      const absoluteSourceTicks = barIndex * ticksPerBarSource + tickInBar
      const t0 = Math.round(absoluteSourceTicks * ppqRatio)
      const t1 = t0 + Math.max(1, Math.round(tickLength * ppqRatio))

      return {
        id: note.id ?? `gen_${idx}`,
        t0,
        t1,
        pitch: Number(note.midiPitch ?? 36),
        vel: Number(note.velocity ?? 96),
        chan: Number(note.channel ?? 10),
        articulationId: note.aspect ?? undefined,
      }
    })
  }

  function convertLegacyMidiNotes(legacyNotes: any[]): MidiNote[] {
    ...
    return legacyNotes.map((n: any, idx: number): MidiNote => {
      const t0 = Math.round((n.time ?? 0) * ticksPerSecond)
      const lengthSec = n.length ?? 0.25
      const t1 = t0 + Math.max(1, Math.round(lengthSec * ticksPerSecond))
            ...
            <div className="col-span-4 space-y-3">
              <DrumCreationPanel
                ...
                onApplyDrums={(payload) => {
                  ...
                  const dcsmNotes = convertDrumTrackNotesToMidi(payload.drum_track)
                  const legacyNotes = convertLegacyMidiNotes(payload.midi_notes || [])
                  const newNotes = dcsmNotes.length ? dcsmNotes : legacyNotes

                  if (newNotes.length === 0) return

                  updateNotes(drumTrackId, drumClipId, newNotes)

                  const currentEnd = clip.endTick ?? 0
                  const maxTick = newNotes.reduce((max, note) => Math.max(max, note.t1), currentEnd)

                  updateClip(drumTrackId, drumClipId, {
                    endTick: maxTick,
                    ...(payload.drum_track ? { dcsmTrack: payload.drum_track } : {}),
                  })
                }}
              />
              {lastConfig && (
