...
export const DrumCreationPanel: React.FC<DrumCreationPanelProps> = ({
  ...
  // UI mode: Basic (fewer controls) vs Advanced (all knobs)
  const [uiMode, setUiMode] = useState<"basic" | "advanced">("basic");

  // Simple persona-aware presets (initially just one tied to Porcaro-style shuffle)
  const drummerPresets: { id: string; label: string; categoryId: string }[] = [
    { id: "studio_shuffle", label: "Studio Shuffle (Porcaro)", categoryId: "studio_rock" },
  ];
  ...
  const [chorusRidePreference, setChorusRidePreference] = useState(0.0);
  ...
  const [styleSourceMode, setStyleSourceMode] = useState<"jamstix" | "signature" | "combined">("combined");
  ...
  // Simple persona "locks" so users can keep Jamstix/persona defaults
  // for whole groups of controls without adding backend complexity yet.
  const [grooveLockedToPersona, setGrooveLockedToPersona] = useState(false);
  const [fillsLockedToPersona, setFillsLockedToPersona] = useState(false);
  ...
  function applyPreset(presetId: string) {
    const preset = drummerPresets.find((p) => p.id === presetId);
    if (!preset) return;

    // Map preset to a drummer category and a set of sensible defaults.
    setDrummerCategoryId(preset.categoryId);

    // These values are tuned for a Porcaro-style studio shuffle feel.
    setIntensity(0.65);
    setVariation(0.45);
    setHumanizeOn(true);
    setHumanizeAmount(0.75);
    setGhostAmount(0.75);
    setSwingAmount(0.25);
    setDrumDensity(0.7);
    setCymbalDensity(0.6);
    setChorusRidePreference(0.6);
    setFillDensity(0.6);
    setArticulationProfile("ghosty");
  }
  ...
  return (
    <div className="space-y-2 p-3 bg-neutral-900 rounded border border-neutral-800 text-[13px] leading-snug">
      ...
      {/* UI mode + simple persona presets */}
      <div className="flex items-center justify-between mb-2 gap-2">
        <div className="inline-flex rounded border border-neutral-700 bg-neutral-950 overflow-hidden text-[11px]">
          <button
            className={`px-2 py-1 ${uiMode === "basic" ? "bg-neutral-800 text-emerald-300" : "text-neutral-300"}`}
            onClick={() => setUiMode("basic")}
          >
            Basic
          </button>
          <button
            className={`px-2 py-1 ${uiMode === "advanced" ? "bg-neutral-800 text-emerald-300" : "text-neutral-300"}`}
        ...
        <div className="flex-1 flex items-center justify-end gap-1 text-[11px]">
          <span className="text-neutral-400 hidden sm:inline">Persona Presets:</span>
          {drummerPresets.map((p) => (
            <button
              key={p.id}
              className="px-2 py-0.5 rounded border border-neutral-700 bg-neutral-950 hover:bg-neutral-800 text-[10px] text-neutral-200"
          ...
          <div className="p-2 space-y-3">
            <div className="flex items-center justify-between text-[11px] text-neutral-400">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={grooveLockedToPersona}
                  onChange={(e) => setGrooveLockedToPersona(e.target.checked)}
                />
                <span>Lock groove to persona defaults</span>
              </div>
            </div>
            {uiMode === "advanced" && (
            ...
            <div className="flex gap-4 mt-2">
              ...
              <KnobCircle
                label="Chorus Ride"
                value={chorusRidePreference}
                onChange={setChorusRidePreference}
              />
            </div>
            <div className="flex gap-4 mt-1 text-[10px] text-neutral-500">
              <div className="w-24">Kick/Snare activity</div>
              <div className="w-24">Hats / Ride / Crashes</div>
              <div className="w-24">Preference for ride in choruses</div>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="p-2 space-y-2">
            <div className="flex items-center justify-between text-[11px] text-neutral-400">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={fillsLockedToPersona}
                  onChange={(e) => setFillsLockedToPersona(e.target.checked)}
                />
                <span>Lock fills to persona defaults</span>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <label className="flex flex-col gap-1">
      ...
      {uiMode === "advanced" && (
        <div className="flex items-center gap-3 text-[11px]">
          ...
          <div className="flex items-center gap-1">
            <span className="text-neutral-400">Style Source</span>
            <select
              className="bg-neutral-950 border border-neutral-700 rounded px-2 py-1"
              value={styleSourceMode}
              onChange={(e) => setStyleSourceMode(e.target.value as any)}
            >
              <option value="combined">Combined</option>
              <option value="jamstix">Jamstix brain</option>
              <option value="signature">Signature songs</option>
            </select>
          </div>
        </div>
      )}

      {uiMode === "advanced" && generationMode === "euclidean" && (
