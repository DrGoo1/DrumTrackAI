...
This patch upgrades `llm_service/app.py` so `/v1/performance_spec` can consume the richer drummer identity exported by the Phase 7 sentient profile pipeline and exposed by the Phase 8 runtime adapter.

## What it changes

- Adds `backend/drummerbrain/performance_spec_sentient.py`
- Enhances `llm_service/app.py`
- Adds `backend/tests/test_phase9_sentient_performance_spec.py`

## Runtime behavior

When `drummer_profile` contains any of the following keys, the service prefers sentient-profile-derived instrument rendering over the generic fallback profiles:
