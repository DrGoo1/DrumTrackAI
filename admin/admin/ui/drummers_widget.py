        ...
        for row in plan_rows:
            try:
                ...
                for song in songs:
                    ...
                    try:
                        self._append_activity(f"Auto-Ingest queued: {title}")
                    except Exception:
                        pass

                    try:
                        self._ai_set_state(title, "QUEUED", progress=0)
                    except Exception:
                        pass
                    ...
                    def _on_download_done(local_path: str, md: Dict[str, Any] = metadata):
                        try:
                            ...
                            try:
                                self._append_activity(f"Auto-Ingest download complete: {os.path.basename(local_path)}")
                            except Exception:
                                pass

                            try:
                                self._ai_set_state(title, "DOWNLOADED", progress=100, details=os.path.basename(local_path))
                            except Exception:
                                pass
                            ...
                            try:
                                self._ai_set_state(title, "FAILED", details=str(e))
                            except Exception:
                                pass
                    ...
                    def _on_download_err(err: str, t: str = title):
                        try:
                            ...
                            try:
                                first_line = (msg.splitlines() or [""])[0].strip()
                                self._ai_set_state(t, "FAILED", details=(first_line or msg)[:240])
                            except Exception:
                                pass
                        except Exception:
                    ...
                    self.youtube_service.download_audio(
                        url,
                        output_path,
                        progress_callback=lambda p, _t=title: self._on_auto_ingest_download_progress(_t, p),
        ...
        try:
            ...
            if success:
                ...
                try:
                    song = None
                    if isinstance(metadata, dict):
                        song = (metadata.get("song_title") or metadata.get("title") or metadata.get("song"))
                    song = (str(song).strip() if song else None) or os.path.splitext(os.path.basename(audio_file_path))[0]
                    self._append_activity(f"Queued to MVSep: {song}")
                    self._ai_set_state(song, "QUEUED_TO_MVSEP")
                except Exception:
                    pass
