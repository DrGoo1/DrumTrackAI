...
class YouTubeDownloadThread(QObject):
    ...
    def download(self):
        ...
        try:
            ...
            try:
                logger.info(f"YouTubeDownloadThread running from: {__file__}")
            except Exception:
                pass
            ...
            try:
                ...
                try:
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        try:
                            logger.info("yt-dlp extract_info starting (download only; conversion handled separately)")
                        except Exception:
                            pass
                ...
                try:
                    ...
                    pass
                try:
                    print("Starting ffmpeg conversion to mp3...")
                except Exception:
                    pass
                ...
                ffmpeg_location_cfg2 = str(
                    os.environ.get('DTK_YTDLP_FFMPEG_LOCATION', '')
                    or os.environ.get('DTK_FFMPEG_LOCATION', '')
                    or ''
                ).strip()
                ffmpeg_location2 = self._detect_ffmpeg_location(ffmpeg_location_cfg2)
                ffmpeg_exe = None
                if ffmpeg_location2:
                    if os.path.isdir(ffmpeg_location2):
                        ffmpeg_exe = os.path.join(ffmpeg_location2, 'ffmpeg.exe')
                        if not os.path.exists(ffmpeg_exe):
                            ffmpeg_exe = None
                    else:
                        ffmpeg_exe = ffmpeg_location2
                if not ffmpeg_exe:
                    ffmpeg_exe = 'ffmpeg'
                ...
                if final_size == 0:
                    raise Exception("Final file is empty after conversion")
