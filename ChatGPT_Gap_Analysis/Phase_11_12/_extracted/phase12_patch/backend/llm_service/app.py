...
def performance_spec(req: PerformanceSpecRequest):
    ...
    for idx, sec in enumerate(sections):
        ...
        if not isinstance(section_phrase_selection, dict) and callable(select_phrase_families):
            ...
            sec_fill = sec.get("transitions", {}).get("fillOut", {}) if isinstance(sec.get("transitions"), dict) else {}
            ...
            section_phrase_selection = select_phrase_families(
                ...
                timekeeper=str(sec_orch.get("timekeeper") or ("ride" if "chorus" in str(label).lower() else "hats")),
                fill_family=str(sec_fill.get("family") or "auto"),
                fill_enabled=bool(sec_fill.get("enabled", True)),
                time_feel=str(sec_timing.get("timeFeel") or global_feel or "straight"),
        ...
        if not isinstance(section_phrase_assets, dict) and callable(retrieve_phrase_assets):
            try:
                ...
                section_phrase_assets = retrieve_phrase_assets(
                    ...
                    timekeeper=str(sec_orch.get("timekeeper") or ("ride" if "chorus" in str(label).lower() else "hats")),
                    bars=max(1, int(bar_end - bar_start + 1)),
                    energy=float(energy),
                )
            except Exception:
        ...
        selected_shape = choose_phrase_shape_from_family(groove_family, fill_family, "swell" if variation > 0.6 else "flat") if callable(choose_phrase_shape_from_family) else ("swell" if variation > 0.6 else "flat")
        fill_bias = 0.0
        try:
            fill_bias = float((section_phrase_selection.get("selectorWeights") or {}).get("fillBias", 0.0))
        except Exception:
            fill_bias = 0.0
        ...
        def _profile_local(inst: str, timing_scale: float, vel_bias: int, accent_boost: int, ghost_reduction: float, rr: int):
            ...
            profile["velocityProfile"]["phraseShape"] = selected_shape
            if groove_family == "ride_lead" and inst == "ride_bow":
                profile["velocityProfile"]["accentBoost"] = int(max(0, min(40, profile["velocityProfile"]["accentBoost"] + 4)))
            if groove_family == "open_hat_lift" and inst == "hihat_closed":
                profile["velocityProfile"]["accentBoost"] = int(max(0, min(40, profile["velocityProfile"]["accentBoost"] + 3)))
            if fill_family in {"linear_burst", "flam_tom", "triplet_turnaround"} and inst == "snare_center":
                profile["flamProbability"] = float(min(0.5, profile.get("flamProbability", 0.0) + 0.08 + fill_bias * 0.12))
                profile["dragProbability"] = float(min(0.35, profile.get("dragProbability", 0.0) + 0.04 + fill_bias * 0.08))
            return profile
        ...
        phrases.append({
            ...
            "barEnd": bar_end,
            "profiles": profiles,
            "phraseSelection": section_phrase_selection,
            "phraseAssets": section_phrase_assets if isinstance(section_phrase_assets, dict) else None,
        })
        ...
        if callable(select_phrase_families):
            try:
                section_payload["phraseSelection"] = select_phrase_families(
                    ...
                    energy=float(energy),
                    variation=float(variation),
                    timekeeper=str(section_payload["orchestration"].get("timekeeper") or "hats"),
                    fill_family=str(section_payload["transitions"]["fillOut"].get("family") or "auto"),
                    fill_enabled=bool(section_payload["transitions"]["fillOut"].get("enabled", True)),
                    time_feel=str(section_payload["timing"].get("timeFeel") or "straight"),
                    drummer_profile=drummer_profile,
                )
            except Exception:
                pass
