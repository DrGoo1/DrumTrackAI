...
def performance_spec(req: PerformanceSpecRequest):
    ...
    section_profile_map = build_section_profile_map(
        songmap_summary.get("sections") if isinstance(songmap_summary, dict) else None,
        drummer_profile,
    )
    ...
    for idx, sec in enumerate(sections):
        ...
        timekeeper_pref = str(local_orchestration_bias.get("preferred_timekeeper") or "hats")
        include_hihat = timekeeper_pref in {"hats", "mixed"}
        include_ride = timekeeper_pref in {"ride", "mixed"}

        profiles = [
            _profile_local("kick", 0.5, 10, int(energy_intensity * 20), 0.7, int(section_humanize_amount * 8)),
            _profile_local("snare_center", 1.0, 0, int(15 + energy_intensity * 25), 0.4, int(section_humanize_amount * 10)),
        ]
        if include_hihat:
            hh = _profile_local("hihat_closed", 0.8, -10, int(8 + energy_intensity * 12), 0.6, int(section_humanize_amount * 6))
            hh["microTiming"]["laidBackAmount"] = 0.28 if local_feel == "laid_back" else (-0.22 if local_feel == "pushed" else hh["microTiming"].get("laidBackAmount", 0.0))
            profiles.append(hh)
        if include_ride:
            ride = _profile_local("ride_bow", 0.7, -5, int(10 + energy_intensity * 15), 0.5, int(section_humanize_amount * 7))
            ride["velocityProfile"]["accentBoost"] = int(min(40, ride["velocityProfile"].get("accentBoost", 0) + local_orchestration_bias.get("ride_bell_probability", 0.0) * 10))
            profiles.append(ride)

        phrase_meta = {
            "sectionSentientOverride": bool(has_sentient_identity(section_profile) or section_profile != drummer_profile),
            "sectionTimeFeel": local_feel,
...
def song_roadmap(req: SongRoadmapRequest):
    ...
    for idx, sec in enumerate(song_sections):
        ...
        if st == "chorus" and (chorus_ride_pref >= 0.25 or energy >= 0.8):
            timekeeper = "ride" if timekeeper != "mixed" else timekeeper
        ...
        roadmap_sections.append(
            {
                ...
                "orchestration": {
                    "timekeeper": timekeeper,
                    "hatOpenBias": max(0.0, min(1.0, 0.5 * (0.15 + energy * 0.35 + swing_amount * 0.15) + 0.5 * float(local_orchestration_bias.get("hat_open_bias", 0.2)))),
                    "rideBellProbability": max(0.0, min(1.0, 0.5 * (0.1 if timekeeper in {"ride", "mixed"} else 0.02) + 0.5 * float(local_orchestration_bias.get("ride_bell_probability", 0.05)))),
                    "crashDownbeatProbability": max(0.0, min(1.0, 0.5 * (0.65 if st in {"chorus", "bridge", "outro"} else 0.35) + 0.5 * float(local_orchestration_bias.get("crash_downbeat_probability", 0.3)))),
                },
                "grooveIntent": {
                ...
                "timing": {
                    ...
                    "humanizeAmount": max(0.0, min(1.0, 0.5 * humanize_amount + 0.5 * float(local_transition_bias.get("humanize_bias", 0.5)))),
                },
                "sentientOverride": {
