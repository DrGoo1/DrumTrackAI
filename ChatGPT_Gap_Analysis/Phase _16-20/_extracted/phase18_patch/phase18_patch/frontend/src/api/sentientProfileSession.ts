...
export async function preloadSentientProfiles(ids: Array<string | null | undefined>): Promise<SentientProfileEntry[]> {
...
export async function attachSentientProfilesWithOverrides(config: DrumGenerationConfig & Record<string, any>): Promise<DrumGenerationConfig> {
  ...
  const sectionIds = Array.from(new Set(sections.map((section: any) => resolveSectionDrummerId(section, baseDrummerId)).filter(Boolean)));
  ...
  return {
    ...(enriched as any),
    sections,
    songSections: Array.isArray((enriched as any).songSections) ? sections : (enriched as any).songSections,
