...
def build_song_roadmap_section_overrides(
    *,
    ...
    hihat_share = _lookup_metric(_lookup_mapping(drummer_profile, "instrument_shares", "instrument_counts"), "hihat", "hat", default=0.0)
    ...
    aggression = _clamp(0.2 + energy * 0.35 + groove_to_fill * 0.30 + (1.0 - pocket) * 0.10 + humanness * 0.05, 0.0, 1.0)
    ...
    if timekeeper == "ride":
        hat_open_bias *= 0.7
    ride_bell_probability = _clamp((0.03 if timekeeper not in {"ride", "mixed"} else 0.10) + groove_to_fill * 0.12 + max(0.0, energy - 0.7) * 0.18, 0.0, 1.0)
    crash_downbeat_probability = _clamp(0.25 + energy * 0.35 + groove_to_fill * 0.20 + (0.08 if st in {"chorus", "bridge", "outro", "ending"} else 0.0), 0.0, 1.0)
    ...
    return {
        ...
        "fillPolicy": {
            ...
            "frequency": "all_transitions" if groove_to_fill >= 0.5 else ("section_transitions" if groove_to_fill >= 0.28 else "conservative"),
