...
def performance_spec(req: PerformanceSpecRequest):
    ...
    if build_dcsm_payload_from_sentient_spec is not None:
        try:
            spec["dcsmRenderPayload"] = build_dcsm_payload_from_sentient_spec(
                spec=spec,
                cfg=cfg,
                style_id=str(style or "rock"),
                resolution_ppq=int(cfg.get("resolutionPpq", 960) or 960),
            )
        except Exception as exc:
            spec["dcsmRenderPayload"] = {
                "available": False,
                "reason": "dcsm_build_error",
                "error": str(exc),
            }
