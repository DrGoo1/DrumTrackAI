    ...
    *,
    ...
    if isinstance(groove, Mapping):
        ...
        if isinstance(inline_steps, Mapping):
            groove_events = _normalize_step_events(
                inline_steps,
                bars=bars,
                aspect="groove",
                source=str(groove_asset_id or "inline"),
            )
        else:
            groove_events = _events_from_midi_path(
                str(groove.get("midiPath") or ""),
                target_bars=bars,
                source=str(groove_asset_id or "midi"),
            )
    ...
    if fill_steps:
        fill_events = _normalize_step_events(
            fill_steps,
            bars=1,
            aspect="fill",
            source=str(fill_asset_id or "fill"),
            start_bar_offset=max(0, bars - 1),
        )
