...
def build_subdivision_offsets(
    ...
    if not rows:
        out = []
        for _i, sub in enumerate(SUBDIVISION_ORDER):
    ...
    aggregate_mean = _weighted_mean(
        [(_safe_float(r.get("mean_offset_ms"), 0.0), _safe_float(r.get("sample_count"), 1.0)) for r in rows],
        0.0,
    )
    aggregate_std = _weighted_mean(
        [(_safe_float(r.get("std_offset_ms"), global_var_ms * 0.7), _safe_float(r.get("sample_count"), 1.0)) for r in rows],
        global_var_ms * 0.7,
    )
    ...
    for r in rows:
        ...
        if sub:
            by_sub.setdefault(sub, []).append(
                (_safe_float(r.get("mean_offset_ms"), aggregate_mean), _safe_float(r.get("sample_count"), 1.0))
            )

    out: List[float] = []
    for _idx, sub in enumerate(SUBDIVISION_ORDER):
        if sub in by_sub:
...
def _dynamic_row(dynamic_container: Any, inst: str, section_label: str) -> Optional[Mapping[str, Any]]:
    ...
    rows = sorted(
        rows,
        key=lambda r: (_safe_float(r.get("sample_count"), 0.0), _safe_float(r.get("velocity_std"), 0.0)),
        reverse=True,
    )
    return rows[0]
...
def build_sentient_instrument_profile(
    ...
    return {
        ...
        "microTiming": {
            ...
            "laidBackAmount": float(
                _clamp(laid_back + (_weighted_mean([(sum(offsets) / len(offsets), 1.0)], 0.0) / 12.0), -1.0, 1.0)
            ),
        ...
        "flamProbability": float(
            _clamp(0.12 if inst == "snare" and sample_count >= 4 and humanize_amount > 0.6 else 0.0, 0.0, 1.0)
        ),
