...
_FALLBACK_FILL_STEPS: Dict[str, Dict[str, List[int]]] = {
    ...
    "rudiment_midi:single_stroke_roll": {
        "snare_center": [0, 4, 8, 12],
        "snare_ghost": [1, 2, 3, 5, 6, 7, 9, 10, 11, 13, 14, 15],
    },
...
def _fill_asset(fill_family: str) -> Dict[str, Any]:
    ...
    if not pattern_steps:
        pattern_steps = dict(
            _FALLBACK_FILL_STEPS.get(fill_id)
            or _FALLBACK_FILL_STEPS.get(resolved_id)
            or _FALLBACK_FILL_STEPS.get("fallback")
            or {}
        )
    return {
        "assetId": fill_id,
