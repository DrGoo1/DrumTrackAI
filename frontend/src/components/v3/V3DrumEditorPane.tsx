...
function barIndexToSec(args: {
  ...
  const { barIndex, beatsPerBar, beatTimes, beatZeroOffsetSec, tempoMap, fallbackBpm } = args;
  const beats = Math.max(0, Math.floor(barIndex)) * Math.max(1, beatsPerBar);

  if (Array.isArray(beatTimes) && beatTimes.length >= 2) {
    return beatsToSecondsFromBeatTimesWithOffset(beatTimes, beats, Number(beatZeroOffsetSec) || 0);
  }
...
function playheadToXPx(args: {
  ...
  const beats = Array.isArray(args.beatTimes) && args.beatTimes.length >= 2
    ? secondsToBeatsFromBeatTimesWithOffset(args.beatTimes, args.tSec, Number(args.beatZeroOffsetSec) || 0)
    : beatsAtTimeFromTempoMap(args.tempoMap || [], args.fallbackBpm, args.tSec);
  return Math.max(0, beats) * pxPerBeat;
}

function formatBBT(args: { tSec: number; timeSig: [number, number]; beatTimes?: number[]; beatZeroOffsetSec?: number; tempoMap?: Array<{ tSec: number; bpm: number }>; fallbackBpm: number }): string {
  const beatsPerBar = Number(args.timeSig?.[0] || 4) || 4;
  const ppq = 960;
  let beats = 0;
  if (Array.isArray(args.beatTimes) && args.beatTimes.length >= 2) {
    beats = secondsToBeatsFromBeatTimesWithOffset(args.beatTimes, args.tSec, Number(args.beatZeroOffsetSec) || 0);
  } else {
...
export function V3DrumEditorPane(props: {
  ...
  const sectionRegions = useMemo(() => {
    ...
    const beatZeroOffsetSec = Number(arrangement.beatZeroOffsetSec) || 0;
    ...
    return secs
      .map((s, idx) => {
        ...
        const startBeats = Array.isArray(arrangement.beatTimes) && arrangement.beatTimes.length >= 2
          ? secondsToBeatsFromBeatTimesWithOffset(arrangement.beatTimes, startSec, beatZeroOffsetSec)
        ...
        const endBeats = Array.isArray(arrangement.beatTimes) && arrangement.beatTimes.length >= 2
          ? secondsToBeatsFromBeatTimesWithOffset(arrangement.beatTimes, endSec, beatZeroOffsetSec)
  ...
  }, [arrangement.beatTimes, arrangement.beatZeroOffsetSec, arrangement.sections, arrangement.tempoMap, arrangement.timeSig, bpm]);
  ...
  }, [arrangement.beatTimes, arrangement.beatZeroOffsetSec, arrangement.tempoMap, bpm, playheadSec, props.pixelsPerBeat]);
  ...
  const totalSongBars = useMemo(() => {
    ...
    for (const s of secs) {
      ...
      const endBeats = Array.isArray(arrangement.beatTimes) && arrangement.beatTimes.length >= 2
        ? timeSecToBeatFloat(arrangement.beatTimes, endSec + beatZeroOffsetSec)
        : beatsAtTimeFromTempoMap(arrangement.tempoMap || [], bpm, endSec);
