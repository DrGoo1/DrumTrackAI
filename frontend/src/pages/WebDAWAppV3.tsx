...
import { TruthProbe, isTruthProbeEnabled, isTruthProbeLoggingEnabled, getWorstBeatTimeAnomaly } from '../audio/truthProbe';
import { getSharedDrumPlayerEngine, DrumPlayerEngine, type DrumPlayerChannelId } from '../audio/drumPlayerEngine';
import { getV3TimelineEngine } from '../audio/v3TimelineEngine';
...
export default function WebDAWAppV3() {
  ...
  const timelineLoadedKeyRef = useRef<string | null>(null);
  const timelineEngineRef = useRef(getV3TimelineEngine());
  const timelineReadyKeyRef = useRef<string | null>(null);
  const drumEngineRef = useRef<DrumPlayerEngine | null>(null);
  ...
  const truthProbeLastLogMsRef = useRef<number>(0);
  const truthProbeScheduledTotalRef = useRef<number>(0);
  const truthProbeLastAnomMsRef = useRef<number>(0);
  const truthProbeLastBeatTimesSigRef = useRef<string>('');

  const truthProbeLastLeadMinSecRef = useRef<number>(0);
  const truthProbeLastLeadAvgSecRef = useRef<number>(0);
  const truthProbeLastLeadCountRef = useRef<number>(0);
  const truthProbeLastLateCountRef = useRef<number>(0);
  const truthProbeLastLateThresholdSecRef = useRef<number>(0.003);

  const truthProbeRunMinLeadSecRef = useRef<number>(Number.POSITIVE_INFINITY);
  const truthProbeRunPeakLateCountRef = useRef<number>(0);
  const truthProbeRunWorstBeatAnomScoreRef = useRef<number>(0);
  const truthProbeRunWorstBeatAnomRef = useRef<any>(null);
  const truthProbeRunStartMsRef = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const hasAudio = Boolean(importState.fileKey && importState.waveform?.duration && timelineReadyKeyRef.current === importState.fileKey);
      const duration = Number(importState.waveform?.duration || 0);
      const hasDuration = Number.isFinite(duration) && duration > 0;
      let t: number | null = null;

      if (hasAudio) {
        const v = timelineEngineRef.current.getCurrentTimeSeconds();
        ...
        if (hasAudio) {
          ...
          if (typeof prev === 'number' && Number.isFinite(prev)) {
            ...
            if (stalledPlayheadFramesRef.current > 20) {
              ...
              setV3Playing(false);
              try {
                timelineEngineRef.current.pause();
              } catch {
                // ignore
              }
              rafPlayRef.current = null;
  ...
  useEffect(() => {
    ...
    if (!key) {
      timelineLoadedKeyRef.current = null;
      timelineReadyKeyRef.current = null;
      return;
    }
    if (timelineLoadedKeyRef.current === key) return;

    let canceled = false;
    (async () => {
      const url = await getDownloadUrlSameOrigin(key);
      if (canceled) return;
      await timelineEngineRef.current.loadUrl(url);
      if (canceled) return;
      timelineLoadedKeyRef.current = key;
  ...
  useEffect(() => {
    ...
    if (!truthProbeRef.current) {
      truthProbeRef.current = new TruthProbe({
        getBpm: () => arrangement.tempoMap?.[0]?.bpm || 120,
        getTimeSignature: () => (arrangement.timeSig as [number, number]) || [4, 4],
        getBeatTimes: () => arrangement.beatTimes,
        scheduleClick: (ctxTime, freq, dur) => {
          const ctx = eng.audioContext;
          if (!ctx) return;
          const now = ctx.currentTime;
          const t0 = Math.max(ctxTime, now + 0.001);
          ...
          gain.gain.exponentialRampToValueAtTime(1.0, t0 + 0.001);
        ...
        report: (stats) => {
          truthProbeScheduledTotalRef.current += stats.scheduled
        },
      });
    }
    ...
    const getNowSec = () => {
      const hasAudio = Boolean(importState.fileKey && importState.waveform?.duration && timelineReadyKeyRef.current === importState.fileKey);
      if (hasAudio) {
        return timelineEngineRef.current.getCurrentTimeSeconds();
      }
      const ctx = eng.audioContext;
      const startCtx = drumOnlyStartCtxTimeRef.current;
      if (ctx && typeof ctx.currentTime === 'number' && Number.isFinite(ctx.currentTime) && typeof startCtx === 'number') {
    ...
    const scheduleWindow = () => {
      ...
      if (isTruthProbeLoggingEnabled()) {
        const nowMs = performance.now();
        if (nowMs - truthProbeLastLogMsRef.current > 1000) {
          truthProbeLastLogMsRef.current = nowMs;

          try {
            const v = window.localStorage.getItem('dtk_truth_late_ms');
            const ms = v ? Number(v) : NaN;
            if (Number.isFinite(ms) && ms >= 0) {
              truthProbeLastLateThresholdSecRef.current = ms / 1000;
            }
          } catch {
            // ignore
          }
          const driftSec = 0;
          console.log('[truth-probe]', {
            hasAudio,
            engineNow,
            ctxNow,
            driftSec,
            scheduledClicksTotal: truthProbeScheduledTotalRef.current,
            drumLeadMinSec: truthProbeLastLeadMinSecRef.current,
            drumLeadAvgSec: truthProbeLastLeadAvgSecRef.current,
            drumLeadCount: truthProbeLastLeadCountRef.current,
            drumLateThresholdSec: truthProbeLastLateThresholdSecRef.current,
            drumLateCount: truthProbeLastLateCountRef.current,
          });
        }

        if (nowMs - truthProbeLastAnomMsRef.current > 2500) {
          truthProbeLastAnomMsRef.current = nowMs;
          const bt = arrangement.beatTimes;
          const beatsPerBar = Number(arrangement.timeSig?.[0] || 4) || 4;
          const sig = Array.isArray(bt) && bt.length ? `${bt.length}:${bt[0]}:${bt[bt.length - 1]}` : '';
          if (sig && sig !== truthProbeLastBeatTimesSigRef.current) {
            truthProbeLastBeatTimesSigRef.current = sig;
          }
          if (Array.isArray(bt) && bt.length >= 3) {
            const worst = getWorstBeatTimeAnomaly({ beatTimes: bt, beatsPerBar });
            if (worst) {
              const score = Math.abs((Number((worst as any).ratioToMedian) || 1) - 1);
              if (score > truthProbeRunWorstBeatAnomScoreRef.current) {
                truthProbeRunWorstBeatAnomScoreRef.current = score;
                truthProbeRunWorstBeatAnomRef.current = worst;
              }
              console.warn('[truth-probe] beatTimes anomaly', {
                beatsPerBar,
                worst,
              });
            }
      ...
      let leadMin = Number.POSITIVE_INFINITY;
      let leadSum = 0;
      let leadCount = 0;
      let lateCount = 0;
      const lateThresholdSec = truthProbeLastLateThresholdSecRef.current;

      const tp = truthProbeRef.current;
      if (tp) {
        const enabled = isTruthProbeEnabled();
        tp.setEnabled(enabled);
        if (enabled && ctx && typeof ctxNow === 'number' && Number.isFinite(ctxNow)) {
          tp.scheduleWindow({
            ctxNow,
            playbackNowSec: engineNow,
            windowStartSec: fromSec,
            windowEndSec: toSec,
          });
        }
      }
      ...
      for (const n of notes) {
        ...
        const whenSec = hasAudio
          ? timelineEngineRef.current.getContextTimeForPlaybackTime(tSec + drumOffsetSec)
          : ctxNow + (tSec - engineNow);
        // Schedule sufficiently ahead to avoid UI-thread jitter.
        const safeWhen = Math.max(ctxNow + 0.03, whenSec);

        const lead = safeWhen - ctxNow;
        if (Number.isFinite(lead) && lead >= 0) {
          leadMin = Math.min(leadMin, lead);
          leadSum += lead;
          leadCount += 1;
          if (lead <= lateThresholdSec) {
            lateCount += 1;
          }
        }
      ...
      if (leadCount > 0) {
        truthProbeLastLeadMinSecRef.current = Number.isFinite(leadMin) ? leadMin : 0;
        truthProbeLastLeadAvgSecRef.current = leadSum / leadCount;
        truthProbeLastLeadCountRef.current = leadCount;
        truthProbeLastLateCountRef.current = lateCount;

        if (Number.isFinite(leadMin)) {
          truthProbeRunMinLeadSecRef.current = Math.min(truthProbeRunMinLeadSecRef.current, leadMin);
        }
        truthProbeRunPeakLateCountRef.current = Math.max(truthProbeRunPeakLateCountRef.current, lateCount);
      } else {
        truthProbeLastLeadMinSecRef.current = 0;
        truthProbeLastLeadAvgSecRef.current = 0;
        truthProbeLastLeadCountRef.current = 0;
        truthProbeLastLateCountRef.current = 0;
      }
  ...
  useEffect(() => {
    if (viewMode === 'legacy') return;
    if (v3Playing) {
      truthProbeRunMinLeadSecRef.current = Number.POSITIVE_INFINITY;
      truthProbeRunPeakLateCountRef.current = 0;
      truthProbeRunWorstBeatAnomScoreRef.current = 0;
      truthProbeRunWorstBeatAnomRef.current = null;
      truthProbeRunStartMsRef.current = performance.now();
      return;
    }

    if (!isTruthProbeLoggingEnabled()) return;
    const started = truthProbeRunStartMsRef.current;
    if (!(started > 0)) return;
    const durMs = Math.max(0, performance.now() - started);

    const minLead = truthProbeRunMinLeadSecRef.current;
    const peakLate = truthProbeRunPeakLateCountRef.current;
    const worstBeat = truthProbeRunWorstBeatAnomRef.current;

    // Dev-only thresholds (overridable via localStorage)
    let warnLatePeak = 1;
    let warnMinLeadMs = 10;
    try {
      const vLate = window.localStorage.getItem('dtk_truth_warn_late_peak');
      const vLead = window.localStorage.getItem('dtk_truth_warn_min_lead_ms');
      const lateN = vLate ? Number(vLate) : NaN;
      const leadMs = vLead ? Number(vLead) : NaN;
      if (Number.isFinite(lateN) && lateN >= 0) warnLatePeak = Math.floor(lateN);
      if (Number.isFinite(leadMs) && leadMs >= 0) warnMinLeadMs = leadMs;
    } catch {
      // ignore
    }
    const warnMinLeadSec = warnMinLeadMs / 1000;

    console.log('[truth-probe] run summary', {
      durationMs: Math.round(durMs),
      drumLeadMinSec: Number.isFinite(minLead) ? minLead : null,
      drumLatePeakCount: peakLate,
      drumLateThresholdSec: truthProbeLastLateThresholdSecRef.current,
      worstBeatTimeAnomaly: worstBeat,
      scheduledClicksTotal: truthProbeScheduledTotalRef.current,
    });

    const minLeadOk = Number.isFinite(minLead) ? minLead : Number.POSITIVE_INFINITY;
    if (peakLate > warnLatePeak) {
      console.warn('[truth-probe] underrun warning', {
        drumLatePeakCount: peakLate,
        warnLatePeak,
        hint: 'Increase lookaheadSec and/or scheduleIntervalMs; avoid scheduling too close to ctxNow.',
      });
    }
    if (minLeadOk < warnMinLeadSec) {
      console.warn('[truth-probe] low lead warning', {
        drumLeadMinSec: minLeadOk,
        warnMinLeadMs,
        hint: 'Lead time is too low; increase lookaheadSec or reduce per-window work.',
      });
    }
  }, [v3Playing, viewMode]);
                            ...
                            if (next) {
                              ...
                              if (hasAudio) {
                                (async () => {
                                  try {
                                    const eng = drumEngineRef.current;
                                    timelineEngineRef.current.play(playheadSec);
                                    if (eng) {
                                      await eng.ensureRunning();
                                    }

                                    lastDrumScheduledToRef.current = timelineEngineRef.current.getCurrentTimeSeconds();
                                    setV3Playing(true);
                                  } catch (e: any) {
                                    useV3Store.getState().setImportState({ error: e?.message || String(e) });
                                    setV3Playing(false);
