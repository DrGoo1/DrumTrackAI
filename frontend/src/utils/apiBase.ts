...
export const CANONICAL_CALIBRATION_API_BASE = "https://drumtrackai-calibration-api2.onrender.com";
...
const isFrontendOriginUrl = (value?: string | null): boolean => {
  ...
  try {
    ...
    return isProductionFrontendHost(hostname);
  } catch {
    return false;
  }
};
...
export function resolveApiBase(): string {
  const explicitBaseRaw = rewriteDeprecatedApiBase(process.env.REACT_APP_API_BASE);
  const explicitBase = isFrontendOriginUrl(explicitBaseRaw) ? "" : explicitBaseRaw;
  ...
  if (typeof window !== "undefined") {
    ...
    const runtimeBaseRaw = rewriteDeprecatedApiBase(win.__API_BASE__);
    const runtimeBase = isFrontendOriginUrl(runtimeBaseRaw) ? "" : runtimeBaseRaw;
    if (runtimeBase) {
      return runtimeBase;
    }
