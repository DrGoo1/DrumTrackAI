...
import { deriveBeatGridFromMaps, type MeterEvent, type TempoEventBB } from "../../time/v3TimelineKernel";
import {
  DEFAULT_INHERIT,
...
function getWaveformDurationSec(): number {
  const wf = useV3Store.getState().importState?.waveform;
  const dur = Number((wf as any)?.duration);
  return Number.isFinite(dur) && dur > 0 ? dur : 0;
}

function recomputeBeatTimesFromMaps(args?: { meterMap?: MeterEvent[]; tempoMapBB?: TempoEventBB[] }) {
  const s = useV3Store.getState();
  const durationSec = getWaveformDurationSec();
  if (!(durationSec > 0)) return;

  const meterMap = args?.meterMap ?? s.arrangement.meterMap;
  const tempoMapBB = args?.tempoMapBB ?? s.arrangement.tempoMapBB;
  const grid = deriveBeatGridFromMaps({ durationSec, meterMap, tempoMapBB, startOffsetSec: 0 });
  s.setBeatTimes(grid.beatTimesSec);
}
...
export const useV3Store = create<V3Store>((set, get) => ({
  ...
  setMeterMap: (meterMap) => {
    set((s) => ({ arrangement: { ...s.arrangement, meterMap: clone(meterMap) } }));
    recomputeBeatTimesFromMaps({ meterMap });
  },
  setTempoMapBB: (tempoMapBB) => {
    set((s) => ({ arrangement: { ...s.arrangement, tempoMapBB: clone(tempoMapBB) } }));
    recomputeBeatTimesFromMaps({ tempoMapBB });
  },
    ...
    set((s) => ({
      arrangement: {
        ...
        beatTimes: (() => {
          ...
          const sorted = asNums.slice().sort((a: number, b: number) => a - b);
          return sorted.length >= 2 ? sorted : undefined;
        })(),
        // Explicit origin: beatTimes[0] is the first detected beat in the audio timeline.
        // Treat that as the beat-grid origin offset relative to audio time-zero.
        beatZeroOffsetSec: (() => {
          if (!beatTimes) return 0;
          const raw = clone(beatTimes) as any;
          if (!Array.isArray(raw) || raw.length < 1) return 0;
  ...
  setTimeSig: (numerator, denominator) => {
    set((s) => ({ arrangement: { ...s.arrangement, timeSig: [numerator, denominator] } }));
    // Keep canonical maps in sync with the legacy timeSig fields for V1.
    // (Meter map is bar-boundary-only; update bar 0 for now.)
    const mm: MeterEvent[] = [{ barIndex: 0, num: numerator, denom: denominator as any }];
    get().setMeterMap(mm);
  },
  setSections: (sections) => set((s) => ({ arrangement: { ...s.arrangement, sections: clone(sections) } })),
  setSectionsBB: (sectionsBB) =>
    set((s) => ({
      arrangement: {
        ...s.arrangement,
        sectionsBB: sectionsBB ? clone(sectionsBB) : [],
      },
    })),
  ...
  setSnapSectionsToBeat: (on) => set((s) => ({ ui: { ...s.ui, snapSectionsToBeat: !!on } })),
    ...
    try {
      ...
      if (!isSectionTarget) {
        ...
        if (
          ...
          nextInherit.humanization = "override";
        }
